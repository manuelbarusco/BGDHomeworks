...
public class G045HW2 {
    ...
    private static long ball_weight(ArrayList<Integer> Z, double r, int index_center, ArrayList<Long> W){
        ...
        for(int j=0;j<distances[0].length; j++){
            if(Z.contains(j) && j!=index_center && distances[index_center][j] <= r )
                ball_weight+=W.get(j);
        }
        return ball_weight;
    }
    ...
    private static ArrayList<Integer> ball(ArrayList<Integer> Z, double r, int center_index){
        ...
        for(int j=0;j<distances[0].length; j++) {
            if (Z.contains(j) && j != center_index && distances[center_index][j] <= r)
                ball.add(j);
        }
            ...
            while (S.size() < k && Wz > 0) {
                ...
                for (int i = 0; i < P.size(); i++) {
                    if(Z.contains((Integer) i)) {
                        if(first_point) {
                            max = ball_weight(Z, (1 + 2 * alpha) * r, i, W);
                            new_center = P.get(i);
                            first_point = false;
                        } else {
                ...
                Z.remove((Integer) P.indexOf(new_center));
                Wz -= W.get(P.indexOf(new_center)); //peso di y
                for (Integer y : ball) {
                    Z.remove(y);
    ...
    public static void main(String[] args) throws IllegalArgumentException, IOException {
        ...
        for (int i=0; i<inputPoints.size(); i++){
            //System.out.println("distances_matrix" + java.util.Arrays.toString(distances[i]));
        ...
        //System.out.println("Centroidi: " + java.util.Arrays.toString(solution.toArray()));
